...
|---|---|
| Dataset name | Breast cancer Wisconsin dataset, as named by `breast_cancer_wisconsin.csv` |
| Dataset source | Not specified in the repository or workflow |
| File location | `data/breast_cancer_wisconsin.csv` |
| Rows | 569 |
| Input features | 30 |
| Target | `diagnosis` |
| Task | Binary classification; the CSV target values are `0` and `1` |
| Download behavior | The dataset is committed directly to the repository; no download step is implemented |
...
This satisfies the assignment because it is a committed, labeled dataset with a target column and a fixed set of numeric input columns for a classification pipeline.
